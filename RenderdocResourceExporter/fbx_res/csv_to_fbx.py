...
class CsvToFbx:
    ...
    def execute(self, csv_path_, fbx_path_, export_config_, alias_config_):
        ...
        # 运行参数格式化
        args = "%s %s %s %s %s %s %s %s " % (bat_path, csv_path_, export_normal, export_tangent, export_color, export_uv, export_uv2, export_uv3)
        # 通过执行命令行脚本来执行exe文件
