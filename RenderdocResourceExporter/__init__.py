...
def ExportFbx(pyrenderdoc_, data_):
    ...
    pyrenderdoc_.Replay().BlockInvoke(meshToCsv.execute(table, csv_save_path, dialog.alias_config))
    # 读取CSV文件，导出FBX文件
    if os.path.exists(csv_save_path):
        csvToFbx = CsvToFbx(emgr)
        pyrenderdoc_.Replay().BlockInvoke(csvToFbx.execute(csv_save_path, fbx_save_path, dialog.export_config, dialog.alias_config))
